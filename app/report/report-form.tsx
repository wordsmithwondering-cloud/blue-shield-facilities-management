...
export default function ReportForm() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    setLoading(false);
    if (response.ok) setTicket(data.ticket);
    else alert(data.error || 'Unable to submit report');
  }

  if (ticket) {
    return <main><div className="card success"><h2>Report Received</h2><p>Your facility issue has been logged successfully.</p><h1>{ticket}</h1><p>Please keep this reference for follow-up.</p></div></main>;
  }

  return <main><div className="card"><h1>Report a Facility Issue</h1><p>Please report defects or facility-related problems at Blue Shield Towers.</p><div className="location">Location: <b>{location}</b></div><br />
